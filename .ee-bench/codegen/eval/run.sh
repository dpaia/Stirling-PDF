...
GRADLE_TEST_ARGS=(--tests "stirling.software.SPDF.config.security.mail.EmailServiceTest")
...
_run_tests() {
  ...
  set +e
  ./gradlew test "${GRADLE_TEST_ARGS[@]}" --no-daemon --continue > "/tmp/${label}_stdout.log" 2> "/tmp/${label}_stderr.log"
  local exit_code=$?
  set -e
